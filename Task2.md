...
Now for invoke and stream
we need to allow user to upload files, and when files are uploaded
we need to create a MediaRef for each file and include that in the message

To Convert pdf docx to text or pass as is as Base64 encoded data
Few APIs support direct file upload, so we need to handle that as well
Keep on flags to indicate whether to convert or not in api

To convert to text you can use this library: https://pypi.org/project/textxtract/
...
Keep this as optional dependency, if user wants to convert files to text they can install it