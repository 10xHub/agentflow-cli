...
All coding agents get the same AgentFlow context. Only the filename and, in a
couple of cases, a small header differ between agents.
"""
...
AGENTFLOW_SKILL_BODY: Final[str] = """\
# AgentFlow
...
| `"google"` | `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) | `gemini-2.0-flash`, `gemini-2.5-flash`, `gemini-2.5-pro` |
| `"vertex_ai"` | `GOOGLE_CLOUD_PROJECT` + ADC (`GOOGLE_APPLICATION_CREDENTIALS` or attached GCP SA) | same Gemini models |
...
- `google-genai` is not bundled. Install via `pip install google-genai` when using either Gemini path.
...
Agent(
    ...
    skills=my_skill_config,              # inject skill docs into system prompt
...
- `PgCheckpointer` / `RedisCheckpointer` for production (requires `REDIS_URL` or a Postgres DSN).
- Always pass `config={"thread_id": "..."}` on `invoke` / `stream` to keep history across calls.
...
CLAUDE_MD: Final[str] = AGENTFLOW_SKILL_BODY

# Cursor picks up .cursor/rules/*.mdc with a small frontmatter header.
CURSOR_MDC: Final[str] = (
    "---\n"
    "description: AgentFlow framework conventions and golden-path snippets\n"
    "alwaysApply: true\n"
    "---\n\n"
    + AGENTFLOW_SKILL_BODY
)

# GitHub Copilot reads .github/copilot-instructions.md.
COPILOT_MD: Final[str] = AGENTFLOW_SKILL_BODY

# Windsurf reads .windsurfrules at the project root.
WINDSURF_RULES: Final[str] = AGENTFLOW_SKILL_BODY

# OpenAI Codex / generic agents read AGENTS.md.
AGENTS_MD: Final[str] = AGENTFLOW_SKILL_BODY


# Registry mapping agent id → (relative path, content)
SKILL_TARGETS: Final[dict[str, tuple[str, str]]] = {
    "claude": ("CLAUDE.md", CLAUDE_MD),
    "cursor": (".cursor/rules/agentflow.mdc", CURSOR_MDC),
    "copilot": (".github/copilot-instructions.md", COPILOT_MD),
    "windsurf": (".windsurfrules", WINDSURF_RULES),
    "codex": ("AGENTS.md", AGENTS_MD),
}
